...
const Index = memo(() => {
  const [
    ...
    { effects },
  ] = useSelector(({ menu, manageSafeZoneAcction, loading }) => [
    menu,
    manageSafeZoneAcction,
    loading,
  ]);
  ...
  useEffect(() => {
    dispatch({
      type: 'manageSafeZoneAcction/GET_DATA',
      payload: {
        user_id: params?.id,
      },
      callback: (response) => {
    ...
    dispatch({
      type: 'manageSafeZoneAcction/GET_STUDENTS',
      payload: {
        class: '',
      },
    });
  ...
  const onFinish = (value) => {
    dispatch({
      type: 'manageSafeZoneAcction/UPDATE',
      payload: {
        ...details,
      ...
      {!isEmpty(details) && (
        <div className={classNames(styles['form-zone'], 'm-3')}>
            ...
            <Pane className="w-100 d-flex flex-row justify-content-between my-3">
              <div className="d-flex flex-column ">
                ...
                <Text size="medium" className={styles['font-weight-bold']}>
                  {Helper.getDate(details?.time_go_in_safe_region, variables.DATE_FORMAT.DATE_VI)}
                </Text>
              </div>
              <div className="d-flex flex-column ">
                <Text size="normal">Vùng</Text>
                <Text size="medium" className={styles['font-weight-bold']}>
                  {details?.name_region}
                </Text>
              </div>
              <div className="d-flex flex-column ">
                ...
                <Text size="medium" className={styles['font-weight-bold']}>
                  {Helper.getDate(details?.time_go_in_safe_region, variables.DATE_FORMAT.TIME_FULL)}
                </Text>
              </div>
            </Pane>
            <div className="mt-2">
          ...
          >
            ...
            <div className={classNames('border-bottom', styles['bg-white'])}>
              <div className="p20">
                ...
                <Table
                  ...
                  dataSource={data}
                  loading={effects['manageSafeZone/GET_STUDENTS']}
            ...
            >
              <Button color="success" loading={effects['manageSafeZoneAcction/UPDATE']} htmlType="submit">Lưu</Button>
            </div>
          </Form>
        </div>
      )}
