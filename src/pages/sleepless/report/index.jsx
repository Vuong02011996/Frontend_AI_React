...
const Index = memo(() => {
  ...
  ] = useSelector(({ loading: { effects }, sleeplessReport }) => [effects, sleeplessReport]);
  ...
  const columns = useMemo(
    () => [
      ...
      {
        title: 'Số ngày trằn trọc',
        width: 180,
        className: 'min-width-180',
        render: (record) => <Text size="normal">{record.num_day}</Text>,
      },
      {
        title: 'Ngày trằn tọc',
        width: 170,
  ...
  const loadClasses = (idBranch) => {
    dispatch({
      type: 'sleeplessReport/GET_CLASSES',
      payload: {
        branch: idBranch,
      },
    });
  ...
  useEffect(() => {
    dispatch({
      type: 'sleeplessReport/GET_BRANCHES',
      payload: {},
    });
  }, []);
  ...
  useEffect(() => {
    dispatch({
      type: 'sleeplessReport/GET_STUDENTS',
      payload: {
        ...
        <Pane className="card mt20">
          <Pane className="p20">
            ...
            <Table
              columns={columns}
              dataSource={data}
              loading={loading['sleeplessReport/GET_DATA']}
