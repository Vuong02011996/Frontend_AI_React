...
import { size, isEmpty, difference, isArray } from 'lodash';
...
const Index = memo(({ streamUrl, statusJob }) => {
  ...
  const [loading, { sleepless, details }] = useSelector(({ loading: { effects }, cameraAdd }) => [
    effects,
    cameraAdd,
  ]);
  ...
  useEffect(() => {
    if (!isEmpty(sleepless) && difference(sleepless, items) && isArray(sleepless)) {
      setItems(sleepless);
    }
  }, [sleepless]);
  ...
  const onChangeLayer = (coordinates, indexLayer) => {
    ...
    dispatch({
      type: 'cameraAdd/UPDATE_SEEPLESS',
  ...
  const onChangeTime = (value, indexLayer, key) => {
    setItems((prev) =>
      prev?.map((item, index) => (index === indexLayer ? { ...item, [key]: value } : item)),
    );
    dispatch({
      type: 'cameraAdd/UPDATE_SEEPLESS',
      payload: items?.map((item, index) =>
        index === indexLayer ? { ...item, [key]: value } : item,
      ),
    });
  };
  ...
  const start = () => {
    dispatch({
      type: 'cameraAdd/START_SLEEPLESS',
      payload: {
        id: params.id,
      },
      callback: (response) => {
  ...
  const onFinish = () => {
    dispatch({
      type: 'cameraAdd/UPDATE_ACTION_SLEEPLESS',
      payload: {
        ...params,
        jobs_cam: {
          sleepless: !isEmpty(sleepless)
            ? sleepless?.map((item) => ({
                ...item,
                from_time: Helper.getDateTime({
                  value: Helper.setDate({
                    ...variables.setDateData,
                    originValue: item.from_time,
                  }),
                  isUTC: false,
                }),
                to_time: Helper.getDateTime({
                  value: Helper.setDate({
                    ...variables.setDateData,
                    originValue: item.to_time,
                  }),
                  isUTC: false,
                }),
              }))
  ...
  return (
    <Pane className="border-bottom pb20 pr20 pl20">
      <div className="d-flex">
        <Switch className="mr10" checked={isShow} onChange={onChangeToggle} />{' '}
        <p>Trằn trọc không ngủ</p>
      </div>
      {isShow && (
        <div className="mt15">
                    ...
                    <div className="ant-row ant-form-item">
                      ...
                      <div className="ant-col ant-form-item-control">
                        <TimePicker
                          ...
                          placeholder="Chọn"
                          minuteStep={15}
                          value={item.from_time && moment(item.from_time)}
                          onChange={(value) => onChangeTime(value, index, 'from_time')}
                        />
                      </div>
                      <div className="ant-col ant-form-item-control pl-5">
                        <TimePicker
                          format={variables.DATE_FORMAT.HOUR}
                          placeholder="Chọn"
                          minuteStep={15}
                          value={item.to_time && moment(item.to_time)}
                          onChange={(value) => onChangeTime(value, index, 'to_time')}
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="d-flex justify-content-end">
                {statusJob.status_job !== 'running' && (
                  <Button
                    ...
                    onClick={start}
                    loading={
                      loading['cameraAdd/START_SLEEPLESS'] || loading['cameraAdd/GET_STATUS_JOBS']
                    }
                  >
                    Start
                  </Button>
                ...
                {statusJob.status_job === 'running' && (
                  <Button
                    ...
                    onClick={stop}
                    loading={
                      loading['cameraAdd/STOP_SLEEPLESS'] || loading['cameraAdd/GET_STATUS_JOBS']
                    }
                  >
                    Stop
                  </Button>
                ...
                <Button
                  ...
                  className="ml10"
                  onClick={onFinish}
                  loading={loading['cameraAdd/UPDATE_ACTION_SLEEPLESS']}
                >
                  Lưu
                </Button>
              </div>
